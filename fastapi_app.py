...
class SpacePlanRequest(BaseModel):
    serviceLineKey: Optional[int] = Field(None, description="Service line key")
    vesselVisitKey: Optional[int] = Field(None, description="VesselVisit dbkey")
    type: int = Field(..., description="Type: 1-create group; 2-create range")
    plan_start_time: Optional[datetime] = None
    plan_end_time: Optional[datetime] = None

    @model_validator(mode="after")
    def validate_target_key(self):
        has_service_line_key = self.serviceLineKey is not None
        has_vessel_visit_key = self.vesselVisitKey is not None
        if has_service_line_key == has_vessel_visit_key:
            raise ValueError("Provide exactly one of serviceLineKey or vesselVisitKey")
        return self
...
def algo_space_plan_request(
    ...
    logger.info(
        f"请求参数: serviceLineKey={request.serviceLineKey}, "
        f"vesselVisitKey={request.vesselVisitKey}, type={request.type}"
    )
    ...
    try:
        ...
        with save_terminal_output("SpacePlan") as algo_log_file:
            result = run_plan(
                token,
                line_keys=request.serviceLineKey,
                vessel_key=request.vesselVisitKey,
                type=request.type,
