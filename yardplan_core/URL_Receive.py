...
# # 别名，兼容 block_list_receive 调用
# block_list_receive = blocklist_receive

    #直接用Yard.json,保存为静态数据
...
def ShipVisitList_receive(token):
    ...
    """
    #======================== 接收数据 ============================
    url_block =  "http://10.28.120.217/optimize/test/getVesselVisit"
    headers = {
        "Authorization": token,
        "Content-Type": "application/json"
    }
...
def SpaceCurrent_receive(token):
    ...
    """
    # ======================== 接收数据 ============================
    url_block = "http://10.28.120.217/optimize/test/getSpaceCurrent"
    headers = {
        "Authorization": token,
        "Content-Type": "application/json"
    }
    response = requests.get(url_block, headers=headers)
    if not response.ok:
        raise Exception(f"获取占用箱位数据失败，状态码：{response.status_code}")

    return response.json()
    

def unit_receive(token,save_path: str = "./data217/unit_data.json"):
    """
    ...
    # with open('./data/unit.json', 'r',encoding="utf-8") as file:
    #     data = json.load(file).get('data')
    # return data
...
def BoundList_receive(token):
    ...
    """
    # ======================== 接收数据 ============================
    url_block = "http://10.28.120.217/optimize/test/getBoundList"
    headers = {
        "Authorization": token,
        "Content-Type": "application/json"
    }
    response = requests.get(url_block, headers=headers)
    if not response.ok:
        raise Exception(f"获取BoundList数据失败，状态码：{response.status_code}")

    return response.json()


def SpaceAllocation_receive(token):
    """
    通过接口获取现有堆存计划信息。
    """
    # ======================== 接收数据 ============================
    url_block = "http://10.28.120.217/optimize/test/getSpaceAllocation"
    headers = {
        "Authorization": token,
        "Content-Type": "application/json"
    }
    response = requests.get(url_block, headers=headers)
    if not response.ok:
        raise Exception(f"获取现有堆存计划数据失败，状态码：{response.status_code}")

    return response.json()

def berthplan_receive(token):
    """
    通过接口获取泊位计划信息。
    """
    # ======================== 接收数据 ============================
    url_block = "http://10.28.120.217/ship/berth/plan/query"
    headers = {
        "Authorization": token,
        "Content-Type": "application/json"
    }
    response = requests.get(url_block, headers=headers)
    if not response.ok:
        raise Exception(f"获取泊位计划数据失败，状态码：{response.status_code}")

    return response.json()

def WqByVesselVisit_receive(token):
    """
    通过接口获取WQ信息。
    """
    # ======================== 接收数据 ============================
    url_block = "http://10.28.120.217/optimize/test/getWqByVesselVisit"
    headers = {
        "Authorization": token,
        "Content-Type": "application/json"
    }
