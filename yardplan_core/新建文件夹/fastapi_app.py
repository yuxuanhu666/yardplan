...
from algo.SpaceAllocation.yardplan.yardplan_core.runtime import run_plan
from algo.AGV.AGV_new import run_agv_scheduler
...
# 设置日志：同时输出到终端和文件
LOG_DIR = os.getenv("ALGO_LOG_DIR", "logs")
os.makedirs(LOG_DIR, exist_ok=True)

SERVICE_LOG_FILE = os.path.join(
    LOG_DIR,
    f"service_{datetime.now().strftime('%Y%m%d')}.log"
)
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(SERVICE_LOG_FILE, encoding="utf-8"),  # 文件
    ],
    force=True,
)
...
@contextmanager
def save_terminal_output(algo_name: str):
    """
    保存一次算法调用期间的 stdout/stderr 到文件，同时不影响终端显示。
    注意：sys.stdout 是进程级变量，所以这里用锁避免并发调用时互相串日志。
    """
    os.makedirs(LOG_DIR, exist_ok=True)

    log_file = os.path.join(
        LOG_DIR,
        f"{algo_name}_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.log"
    )
    ...
    with _stdout_lock:
        ...
        with open(log_file, "a", encoding="utf-8") as f:
            ...
            try:
                logger.info(f"{algo_name} 本次算法输出日志文件: {log_file}")
                yield log_file
            finally:
...
def build_public_url(path: str, use_secondary_base: bool = False) -> str:
    base_url = (
        service_config.ALGO_PUBLIC_BASE_URL_SECONDARY
        if use_secondary_base
        else service_config.ALGO_PUBLIC_BASE_URL
    )
...
MODULE_REGISTER_CONFIG = {
    "dualCycle": {
        ...
        "version": "V1.0.0",
        "status": "1",
        "url": build_public_url("/algo/twinCycle/request"),
        "note": "华东理工大学-边装边卸算法",
        "isCurrent": 1
    },
    "Expertdecking":{
        ...
        "version": "V1.0.0",
        "status": "1",
        "url": build_public_url("/module/contr/slot/request"),
        "note": "华东理工大学-专家派位算法",
        "isCurrent": 1
    },
    "YcScheduler":{
        ...
        "version": "V1.0.0",
        "status": "1",
        "url": build_public_url("/module/contr/yc/scheduler/request"),
    ...
    "SpacePlan":{
        ...
        "version": "V1.0.0",
        "status": "1",
        "url": build_public_url("/module/contr/spaceplan/scheduler/request"),
    ...
    "QcScheduler": {
        ...
        "version": "V1.0.0",
        "status": "1",
        "url": build_public_url("/module/contr/qc/scheduler/request", use_secondary_base=True),
        "note": "华东理工大学-工班计划算法",
        "isCurrent": 1
    },
    "AgvScheduler": {
        ...
        "version": "V1.0.0",
        "status": "1",
        "url": build_public_url("/module/contr/agv/scheduler/request", use_secondary_base=True),
        "note": "华东理工大学-集卡池分配算法",
        "isCurrent": 1
    }
}
...
def get_token(algo_name: str) -> Optional[str]:
    ...
    register_url = service_config.ALGO_REGISTER_URL
    ...
    try:
        ...
        logger.info(f"{algo_name} Token 获取成功: {token}, register_url={register_url}")
        return token
...
class SpacePlanImportInfo(BaseModel):
    blockIdList: List[str] = Field(
        default_factory=list,
        description="Allowed import yard block IDs; empty means use default import blocks",
    )
    cnt20Ft: int = Field(..., ge=0, description="Minimum 20ft import count")
    cnt40Ft: int = Field(..., ge=0, description="Minimum 40ft import count")


class SpacePlanPodInfo(BaseModel):
    pod: str = Field(..., min_length=1, description="Port of discharge")
    cnt20Ft: int = Field(..., ge=0, description="Minimum 20ft export count")
    cnt40Ft: int = Field(..., ge=0, description="Minimum 40ft export count")


class SpacePlanExportInfo(BaseModel):
    blockIdList: List[str] = Field(
        ...,
        min_length=1,
        description="Allowed export yard block IDs",
    )
    podList: List[SpacePlanPodInfo] = Field(
        ...,
        min_length=1,
        description="Export demand by POD",
    )


class SpacePlanRequest(BaseModel):
    vesselVisitKeyList: Optional[List[int]] = Field(
        None,
        description="VesselVisit dbkey list; currently exactly one item is supported",
    )
    vesselLineKeyList: Optional[List[int]] = Field(
        None,
        description="Vessel line key list; currently exactly one item is supported",
    )
    importInfo: SpacePlanImportInfo
    exportInfo: SpacePlanExportInfo
    isAutoGroup: int = Field(..., ge=0, le=1)
    isAutoRange: int = Field(..., ge=0, le=1)

    @model_validator(mode="after")
    def validate_space_plan_request(self):
        has_line_key = bool(self.vesselLineKeyList)
        has_visit_key = bool(self.vesselVisitKeyList)
        if has_line_key == has_visit_key:
            raise ValueError(
                "Provide exactly one of vesselLineKeyList or vesselVisitKeyList"
            )
        selected_keys = (
            self.vesselLineKeyList if has_line_key else self.vesselVisitKeyList
        )
        if selected_keys is None or len(selected_keys) != 1:
            raise ValueError("Currently exactly one vessel/line key is supported")
        if self.isAutoGroup + self.isAutoRange != 1:
...
async def algo_agv_scheduler_request(
    ...
    _: Any = Depends(
        lambda authorization=Header(None):
        verify_token("AgvScheduler", authorization)
    )
):
    ...
    logger.info(
        f"请求参数: powKeyList={request.powKeyList}"
    )

    try:

        token = get_module_token("AgvScheduler")

        result = run_agv_scheduler(
            token,
            request.powKeyList
        )

        pooling_result = result["pooling_result"]

        # 保持和原接口一致
        # 每2个pow分一组
        data = []

        pow_keys = list(pooling_result.keys())

        for i in range(0, len(pow_keys), 2):

            group = {}

            for key in pow_keys[i:i + 2]:

                group[key] = pooling_result[key]
...
def algo_space_plan_request(
    ...
    logger.info(
        f"请求参数: vesselLineKeyList={request.vesselLineKeyList}, "
        f"vesselVisitKeyList={request.vesselVisitKeyList}, "
        f"isAutoGroup={request.isAutoGroup}, isAutoRange={request.isAutoRange}"
    )

    try:
        token = get_module_token("SpacePlan")
        with save_terminal_output("SpacePlan") as algo_log_file:
            result = run_plan(
                token,
                vessel_line_keys=request.vesselLineKeyList,
                vessel_visit_keys=request.vesselVisitKeyList,
                import_info=request.importInfo.model_dump(),
                export_info=request.exportInfo.model_dump(),
                is_auto_group=request.isAutoGroup,
                is_auto_range=request.isAutoRange,
            )

        range_items = result.metrics.get("range_plan", {}).get("data", [])

        logger.info(f"SpacePlan 算法调用完成，日志已保存: {algo_log_file}")
...
def hello_on_startup():
    ...
    print(f"/--***--       port={service_config.SERVICE_PORT} platform={service_config.TOS_BASE_URL}       --***--/")
    print(f"/--***--       public={service_config.ALGO_PUBLIC_BASE_URL}       --***--/")
